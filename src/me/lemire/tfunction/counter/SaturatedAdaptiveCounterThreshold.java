...
public class SaturatedAdaptiveCounterThreshold implements ThresholdFunction {
        @Override
        ...
        private static int[] threshold8(int min, int[]... set) {
                ...
                for (int k = 0; k < set.length; ++k)
                        if (set[k].length > 0)
                                if (set[k][set[k].length - 1] >= maxl)
                                        maxl = set[k][set[k].length - 1] + 1;
                byte[] counter = new byte[maxl];
                for (int k = 0; k < set.length; ++k) {
                        for (int i : set[k])
                                if(counter[i] < min) counter[i]++;
                }
                int pos = 0;
        ...
        @Override
        public String name() {
                return "SaturatedAdaptiveScanCount";
        }
}
