 ...
 *
 */
public class BitmapDivideSkip implements IndexableThresholdFunction{
   HashMap<int[], BitSet> index = new HashMap<int[], BitSet>();
    ...
    public BitmapDivideSkip( int L) {
        this.USER_L = L;
    }
    public BitmapDivideSkip (double mu) {
        this.mu = mu;
    }
    private BitmapDivideSkip(){}  // want either L or mu, please
	...
	public int[] threshold(final int min, final int[]... set) {
		...
		BitSet[] longlistsb = new BitSet[L];
		for(int k = 0; k<L; ++k) {
			longlistsb[k] = index.get(set[k]);
		}
		...
		int pos = 0;
                while (h.size() >= min-L) { // still could hit min
                    ...
                    if(n>=min-L) {
                        // maybe it can fit...
                        if (isBeyondThreshold(t.value(), longlistsb,n,min)) {
	...
	public String name() {
            if (USER_L != IMPOSSIBLE)
		return "BitmapDivideSkip("+USER_L+")";
            else return "BitmapDivideSkip(u="+mu+")";
	}
        @Override
